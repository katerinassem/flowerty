...
public class NewStateStrategy implements AvailableStatesStrategy {
    ...
    public List<State> getAvailableOrderStates(String roleDescription) {
        ...
        if(StringUtils.equalsIgnoreCase(roleDescription, Role.ROLE_TYPE.ORDERS_MANAGER.toString())) {
            states.add(new State(State.DESCRIPTION_TYPE.ACCEPTED));
        }
        return states;
    }
}
